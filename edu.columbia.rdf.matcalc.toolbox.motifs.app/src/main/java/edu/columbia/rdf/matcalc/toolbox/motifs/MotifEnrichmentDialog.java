...
public class MotifEnrichmentDialog extends ModernDialogHelpWindow {
	...
	private ModernRadioButton mCheckVs =
			new ModernRadioButton("Foreground vs Background", true, ModernWidget.EXTRA_LARGE_SIZE);
	
	private ModernRadioButton mCheckRandomBackground =
			new ModernRadioButton("Random Background");
	
	//private ModernTextField mExt5pField =
	//		new ModernNumericalTextField(SettingsService.getInstance().getAsInt("motifs.search.5p-extension"));
	
	//private ModernTextField mExt3pField =
	//		new ModernNumericalTextField(SettingsService.getInstance().getAsInt("motifs.search.3p-extension"));
	
	private ModernCompactSpinner mSensField =
			new ModernCompactSpinner(0, 1, SettingsService.getInstance().getAsDouble("motifs.minimum-sensitivity"), 0.1);
	
	private ModernCompactSpinner mSpecField =
			new ModernCompactSpinner(0, 1, SettingsService.getInstance().getAsDouble("motifs.minimum-specificity"), 0.1);
	
	private ModernCompactSpinner mThresholdField =
			new ModernCompactSpinner(0, 1, SettingsService.getInstance().getAsDouble("motifs.motif-threshold"), 0.1);
	
	private ModernCheckBox mCheckMainVariants = 
			new ModernCheckBox("Main gene variants");
	
	private ModernCheckBox mCheckPeakWidths = 
	...
	private class CheckAllEvents implements ModernClickListener {
		...
		public void clicked(ModernClickEvent e) {
			for (ModernCheckBox checkBox : mMotifMap.keySet()) {
	...
	*/
	...
	private final void createUi() {
		...
		ModernScrollPane scrollPane = new ModernScrollPane(new FigurePanel(new Figure("Seq Logo Canvas")
				.addSubFigure(mCanvasPanel)))
		...
		for (Motif m : mMotifs) {
			ModernCheckBox checkBox = new ModernCheckBox(m.getName() + " (" + m.getId() + ")");
	...
	public final void clicked(ModernClickEvent e) {
		if (e.getMessage().equals(UI.BUTTON_OK)) {
			try {
				...
				//SettingsService.getInstance().update("motifs.search.5p-extension", mExt5pField.getText());
				//SettingsService.getInstance().update("motifs.search.3p-extension", mExt5pField.getText());
	...
	public List<Motif> getMotifs() {
		...
		for (ModernCheckBox checkBox : mMotifMap.keySet()) {
			if (checkBox.isSelected()) {
				ret.add(mMotifMap.get(checkBox));
			}
		}
	...
	*/
	...
	//	return (MainMatCalcWindow)WindowService.getInstance().findByName(mForegroundCombo.getText());
	//}
	...
	public boolean getUsePeakWidths() {
		return mCheckPeakWidths.isSelected();
	}
	*/
