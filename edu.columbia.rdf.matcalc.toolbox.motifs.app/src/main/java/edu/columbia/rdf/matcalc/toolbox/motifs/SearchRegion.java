...
public class SearchRegion {
	...
	public SearchRegion(String name, 
	...
	/**
	 * The point around which searches are conducted, e.g. the mid point
	 * of a peak or the TSS of a gene.
	 * 
	 * @return
	 */
	...
	public static SearchRegion createSearchRegion(GenomicRegion region, 
			int ext5p, 
			int ext3p) {
		return new SearchRegion(region.getLocation(), 
				region, 
				GenomicRegion.midRegion(region), 
				Strand.SENSE,
				ext5p,
				ext3p);
	}
